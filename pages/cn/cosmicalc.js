...
$("#VERSION").text("CosmiCalc [CN] Ver."+VERSION);
...
var part_func = function(i, part){
	...
	if(part!="fixed"){
		if(Parts_data[pd.name]) throw "名字重複";
		...
		if(Parts_data.fixed[pd.name]) throw "名字重複";
...
$("#TIO_OUT").click(function(){
	...
	str += "\n【組合檢查】\n";
	str += "COST： "+(Result.capa < Result.cost ? "超過容量" : "OK")+"\n";

	$.each(["BS","LG"], function(i, p){
		str += p+"： "+(Result[p.toLowerCase()] ? "OK" : "並未安裝")+"\n";
	});
...
$("#TIO_READ").click(function(){
	if(Prop.changed){
		if(!confirm("組合已變更\n將會進行覆蓋,確定嗎?")) return false;
		Prop.changed = false;
	}
	var str = $("#TIO_TEXTAREA")[0].value,
		parts = str.match(/【零件組合】[^【]+/)+"",
		cartridge = (str.match(/【卡片】[^【]+/)+"").split("\n"),
		ccd = {part:[],cartridge:[]}, i = 0, tmp;

	if(parts!="null"){
		ccd.name = (str.match(/【名字】[^【]+/)+"").split("\n")[1] || "";
		...
		parts.replace(/(BD|WB|LG|HD|BS|AM|WP|HDAC|FCAC)[:：][ 　\t]*([^\r\n]+)/g, function(str, p1, p2){
			...
			$.each(p2.split(/[ 　\t]+/), function(){
				if(this[0] == "["){
					...
					if(Master.tuneup[tn] == undefined) Message.set(tn+" 找不到強化資料", 1);
		...
		$.each(cartridge, function(i){
			if(i){
				...
				if(str != ""){
					...
					else Message.set(str+" 找不到卡片資料", 1);
				}
			}
		});
	}else{
...
$("#SS_CLEAR").click(function(){
	if(confirm("清除 slot"+save_slot+" 的保存資料嗎?")){
		...
		Message.set("已消除 slot"+Settings.save_slot+" 的資料");
	}
});
...
$.each(["TEC","STR"], function(i, s){
	var str='<tr><th class="th1"><div class="right">'+s+'</div><div class="left">傷害</div></th>',
	...
	try{
		...
		function check_str(obj){
			if(!str.match(obj ? /^[a-z]\w+:(?:-?\d+|"[^\"\n]*")$/ : /^(-?\d+|"[^\"\n]*")$/))
				errmsg.push("　　無法解讀'"+str+"' 的資料");
		};

		try{
			$.each(data, function(i, c){
				if(i > MAX_DATA_SIZE) throw "因資料量過大已結束解讀";
				if(fstr){
					 str += c;
					 if(c=='"') fstr = 0;
				}else{
					...
					case '\n':
						if(errmsg.length){
							errmsg.unshift('JSON文法錯誤!　'+fname+'.json Line:'+line);
							errcnt += errmsg.length-1;
							if(errcnt > MAX_ERROR_COUNT) throw "因錯誤過多已結束解讀";
					...
					case '}':
						...
						break;
					case '[':
						if(bc != ':') errmsg.push('　　無法解讀 "'+(str || bc)+c+'" 的資料');
						brackets.push(0);
						b2++;
						str = "";
						bc = c;
	...
	this.exec = function(callback){
		...
		for(tcount=0,i=0; i<task.length; i++) (function(fname, cb){
			var webAccess = INI.webAccess && INI.accessData[fname],
				dataError = function(){
					Message.set('Error! 無法讀取 '+fname, 2);
			...
			expandData = function(data){
				if(data = evalJSON(secureJSON(data), fname)){
					Message.set('已取得 '+fname);
					$.each(data, function(i){
						try{
							cb.call(this, i, fname.indexOf("wp")>0 ? "wp" : fname);
						}catch(e){
							Message.set('警告! file:'+fname+'.json　line:'+i+'　"'+this.name+'"因 '+e+'的錯誤而無法讀取', 1);
						}
					});
					Message.set("　載入完成。");
				}
			...
			getLocal = function(){
				if(webAccess){
					Message.set("無法讀取 "+fname+" ,開始讀取本地資料", 1);
					$.ajax({
...
function typeToClass(typ){
	var cls = [];
	typ.replace(/[陸空砲輔]/g, function(str){
		cls.push({"陸":"riku", "空":"ku", "砲":"hou", "輔":"ho"}[str]);
	});
	return cls.join(" ");
};
...
function setDefault(obj, part, cmn, ss){
	...
	if(obj.type){
		var typ = "";
		if(part=="bd"){
			if(obj.type.match(/[陸空砲輔]/)) obj.type = RegExp.lastMatch;
			else obj.type = {l:"陸",a:"空",c:"砲",s:"輔"}[obj.type] || "－";
		}else{
			var typ = obj.type.match(/[l陸]/) ? "陸" : "－";
			typ += obj.type.match(/[a空]/) ? "空" : "－";
			typ += obj.type.match(/[c砲]/) ? "砲" : "－";
			typ += obj.type.match(/[s輔]/) ? "輔" : "－";
			obj.type = typ;
		}
...
function autoSetup(ccd){
	...
	$.each(ccd.part, function(i){
		...
		if(elm.length) sb = elm[0].ccd_sb;
			...
			Message.set('無法於 '+this.part.toUpperCase()+' 裝備 "'+this.name+'"', 1);
			return;
		}
	...
	if(Result.lv < j) Message.set("有無法讀取的卡片存在", 1);
	$("#CURRENT_LV").text(Result.lv+"/10");
