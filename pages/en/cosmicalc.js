...
$("#VERSION").text("CosmiCalc [EN] Ver."+VERSION);
...
//		if(INI.browsercheck && !$.browser.mozilla && !$.browser.opera && !$.browser.webkit)
		if(INI.browsercheck && !$.browser.mozilla)
			alert("Calc is not compatible with your browser. \n Calc my not work properly.");
	}
});
...
var part_func = function(i, part){
	...
	if(Parts_data[part][this.id]) throw "Duplicate ID";
	pd = Parts_data[part][this.id] = setDefault(this, part, cmn_stat);

	if(part!="fixed"){
		if(Parts_data[pd.name]) throw "Duplicate Name";
		...
		if(Parts_data.fixed[pd.name]) throw "Duplicate Name";
...
ajaxManager.set({
	...
	callback: function(i){
		...
		if(td.type){
			var typ = td.type.match(/[lLND]/) ? "LND" : "－";
			typ += td.type.match(/[aAIR]/) ? "AIR" : "－";
			typ += td.type.match(/[cART]/) ? "ART" : "－";
			typ += td.type.match(/[sSUP]/) ? "SUP" : "－";
			if(typ == "－－－－") throw "type";
			td.type = typ;
		}else td.type = "";
...
$(".parts_list:not('#LIST_BD')").click(function(e){
	...
	if(pd.jointPart){
		if(ccd.parts_ref != Parts_none && ccd.parts_ref != pd && !ccd.parts_ref.jointPart){
			if(confirm(pd.name+" Are you sure you want to insert xxx in this position?\nIf you select \"Cancel\", the current part will be replaced.")){
...
$("#TIO_OUT").click(function(){
	...
	str += "\n【Configuration Check】\n";
	str += "COST： "+(Result.capa < Result.cost ? "Over Cost" : "OK")+"\n";

	$.each(["BS","LG"], function(i, p){
		str += p+"： "+(Result[p.toLowerCase()] ? "OK" : "Not Installed.")+"\n";
	});
...
$("#TIO_READ").click(function(){
	if(Prop.changed){
		if(!confirm("The configuration has been changed.\nIf you proceed previous save will be overwritten. Is that okay?")) return false;
		Prop.changed = false;
	}
	var str = $("#TIO_TEXTAREA")[0].value,
		parts = str.match(/【Parts Configuration】[^【]+/)+"",
		cartridge = (str.match(/【Cartridge】[^【]+/)+"").split("\n"),
		ccd = {part:[],cartridge:[]}, i = 0, tmp;

	if(parts!="null"){
		ccd.name = (str.match(/【Robo Name】[^【]+/)+"").split("\n")[1] || "";
		...
		parts.replace(/(BD|WB|LG|HD|BS|AM|WP|HDAC|FCAC)[:：][ 　\t]*([^\r\n]+)/g, function(str, p1, p2){
			...
			$.each(p2.split(/[ 　\t]+/), function(){
				if(this[0] == "["){
					...
					if(Master.tuneup[tn] == undefined) Message.set(tn+" No Tune Data Found.", 1);
		...
		$.each(cartridge, function(i){
			if(i){
				...
				if(str != ""){
					...
					else Message.set(str+" No Cartridge Data Found.", 1);
				}
			}
		});
	}else{
	...
	if(ccd.part.length){
		autoSetup(ccd);
		Message.set("Configuration loaded.");
...
$("#SS_SAVE").click(function(){
	...
	if(Assembly.bd.sb.ccd.parts_ref == Parts_none){
		if(ck){
			if(confirm("slot"+save_slot+" Data will be deleted. Are you sure?")){
				clearCookie(save_slot);
				$(".ss_item:eq("+save_slot+")").text("slot "+save_slot+": no data");
			}
		}else alert("After finishing Build don't forget to save.");
		return false;
	}
	if(ck != "" && !confirm("It will overwrite saved data. Are you sure?")) return false;

	if(setCookie(save_slot, compOut())){
		$("#SS_CURRENT_VAL, .ss_item:eq("+save_slot+")").text("slot "+save_slot+": "+Assembly.robo_name.text);
		Message.set("The configuration has been saved.");
	}else Message.set("Failed to save the configuration.", 1);
	Prop.changed = false;
});
...
$("#SS_CLEAR").click(function(){
	if(confirm("slot"+Settings.save_slot+" Data will be deleted. Are you sure?")){
		clearCookie(Settings.save_slot);
		$("#SS_CURRENT_VAL, .ss_item:eq("+Settings.save_slot+")").text("slot "+Settings.save_slot+": no data");
		Message.set("slot"+Settings.save_slot+" Stored Data has Been Deleted.");
	}
});
...
$.each(["TEC","STR"], function(i, s){
	var str='<tr><th class="th1"><div class="right">'+s+'</div><div class="left">Force</div></th>',
	...
	try{
		...
		try{
			$.each(data, function(i, c){
				if(i > MAX_DATA_SIZE) throw "Analysis is terminated because the data is too large.";
				if(fstr){
					 str += c;
					 if(c=='"') fstr = 0;
				}else{
					...
					case '\n':
						if(errmsg.length){
							errmsg.unshift('JSON Parts Error!　'+fname+'.json Line:'+line);
							errcnt += errmsg.length-1;
							if(errcnt > MAX_ERROR_COUNT) throw "The analysis will end because there are too many errors.";
					...
					case '}':
						...
						break;
					case '[':
						if(bc != ':') errmsg.push('　　"'+(str || bc)+c+'" Couldn\'t be Ended.');
						brackets.push(0);
						b2++;
						str = "";
						bc = c;
		...
		for(tcount=0,i=0; i<task.length; i++) (function(fname, cb){
			...
			expandData = function(data){
				if(data = evalJSON(secureJSON(data), fname)){
					Message.set(fname+" Retrived.");
					$.each(data, function(i){
						try{
							cb.call(this, i, fname.indexOf("wp")>0 ? "wp" : fname);
						}catch(e){
							Message.set('WARNING!　file:'+fname+'.json　line:'+i+'　"'+this.name+'" Couldn\'t be loaded because '+e+' Error. ', 1);
						}
					});
			...
			getLocal = function(){
				if(webAccess){
					Message.set(fname+" Couldn't Load use local data.", 1);
					$.ajax({
...
function setCookie(key, val, tmp, ck) {
	...
	if(document.cookie.length + tmp.length - ck.length > 8192){
		alert('You have exceeded the allowable storage size for cookies.\nYou have exceeded the allowable storage size for cookies.');
		return false;
	}
...
function typeToClass(typ){
	var cls = [];
	typ.replace(/[LNDAIRARTSUP]/g, function(str){
		cls.push({"LND":"riku", "AIR":"ku", "ART":"hou", "SUP":"ho"}[str]);
	});
	return cls.join(" ");
};
...
function setDefault(obj, part, cmn, ss){
	...
	if(obj.type){
		var typ = "";
		if(part=="bd"){
			if(obj.type.match(/[LNDAIRARTSUP]/)) obj.type = RegExp.lastMatch;
			else obj.type = {l:"LND",a:"AIR",c:"ART",s:"SUP"}[obj.type] || "－";
		}else{
			var typ = obj.type.match(/[lLND]/) ? "LND" : "－";
			typ += obj.type.match(/[aAIR]/) ? "AIR" : "－";
			typ += obj.type.match(/[cART]/) ? "ART" : "－";
			typ += obj.type.match(/[sSUP]/) ? "SUP" : "－";
			obj.type = typ;
		}
...
function autoSetup(ccd){
	...
	$.each(ccd.part, function(i){
		...
		if(elm.length) sb = elm[0].ccd_sb;
			...
			Message.set(this.part.toUpperCase()+' Can\'t attach to "'+this.name, 1);
			return;
		}
		...
		if(this.part == "bd"){
			...
			if(pd.type.indexOf(bdpd.type) < 0) str = 'Type';
			if(SIZE_TO_NUMLIC[pd.size] !== "" &&
				Math.abs(SIZE_TO_NUMLIC[bdpd.size] - SIZE_TO_NUMLIC[pd.size]) > 1){
					str += (str ? '、' : "") + 'Size';
			}
			if(str){
				Message.set('"'+this.name+'Can\'t be used due to '+str+' Restrictions.', 1);
				return;
			}
		}
	...
	if(Result.lv < j) Message.set("There is a cartridge that couldn't be loaded.", 1);
	$("#CURRENT_LV").text(Result.lv+"/10");
