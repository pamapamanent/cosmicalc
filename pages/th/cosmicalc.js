...
$("#VERSION").text("CosmiCalc [TH] Ver."+VERSION);
...
//		if(INI.browsercheck && !$.browser.mozilla && !$.browser.opera && !$.browser.webkit)
		if(INI.browsercheck && !$.browser.mozilla)
			alert("เบราว์เซอร์ของคุณไม่รองรับเครื่องมือคำนวน");
	}
});
...
var part_func = function(i, part){
	...
	if(Parts_data[part][this.id]) throw "สำเนาไอดี";
	pd = Parts_data[part][this.id] = setDefault(this, part, cmn_stat);

	if(part!="fixed"){
		if(Parts_data[pd.name]) throw "สำเนาชื่อ";
		...
		if(Parts_data.fixed[pd.name]) throw "สำเนาชื่อ";
...
ajaxManager.set({
	...
	callback: function(i){
		...
		if(td.type){
			var typ = td.type.match(/[lแลนด์]/) ? "แลนด์" : "－";
			typ += td.type.match(/[aแอร์]/) ? "แอร์" : "－";
			typ += td.type.match(/[cอาร์ท]/) ? "อาร์ท" : "－";
			typ += td.type.match(/[sซัพ]/) ? "ซัพ" : "－";
			if(typ == "－－－－") throw "type";
			td.type = typ;
		}else td.type = "";
...
$(".parts_list:not('#LIST_BD')").click(function(e){
	...
	if(pd.jointPart){
		if(ccd.parts_ref != Parts_none && ccd.parts_ref != pd && !ccd.parts_ref.jointPart){
			if(confirm("ยืนยันการใส่พาร์ท "+pd.name+" เข้ามา \nหากคุณกดยกเลิก พาร์ทปัจจุบันจะถูกเปลี่ยนทับ")){
...
$(".select_box").each(function(){
	...
	if(val[0] < "a" || val.charCodeAt(0) > 256){
		...
		val.toLowerCase().replace(/type *= *([lacs])/g, function(str, v){
			v = {l:"แลนด์",a:"แอร์",c:"อาร์ท",s:"ซัพ"}[v];
...
$("#TIO_OUT").click(function(){
	...
	str += "\n【ตรวจสอบการปรับแต่ง】\n";
	str += "COST： "+(Result.capa < Result.cost ? "คอสท์เกิน" : "OK")+"\n";

	$.each(["BS","LG"], function(i, p){
		str += p+"： "+(Result[p.toLowerCase()] ? "OK" : "ไม่ได้ติดตั้งไว้")+"\n";
	});
...
$("#TIO_READ").click(function(){
	if(Prop.changed){
		if(!confirm("การตั้งค่าถูกปรับเปลี่ยนเรียบร้อย \nยืนยันการเซฟทับไฟล์เก่า")) return false;
		Prop.changed = false;
	}
		...
		parts.replace(/(BD|WB|LG|HD|BS|AM|WP|HDAC|FCAC)[:：][ 　\t]*([^\r\n]+)/g, function(str, p1, p2){
			...
			$.each(p2.split(/[ 　\t]+/), function(){
				if(this[0] == "["){
					...
					if(Master.tuneup[tn] == undefined) Message.set("ไม่พบข้อมูลการจูนสำหรับ "+tn, 1);
		...
		$.each(cartridge, function(i){
			if(i){
				...
				if(str != ""){
					...
					else Message.set("ไม่พบข้อมูลคาร์ทริดจ์สำหรับ "+str, 1);
				}
			}
		});
	}else{
	...
	if(ccd.part.length){
		autoSetup(ccd);
		Message.set("โหลดข้อมูลการปรับแต่งสำเร็จ");
		$(this).parents(".transparent_field").hide();
	}else{
	 Message.set("โหลดข้อมูลการปรับแต่งล้มเหลว", 1);
	}
	Prop.changed = false;
});
...
$("#SS_SAVE").click(function(){
	...
	if(Assembly.bd.sb.ccd.parts_ref == Parts_none){
		if(ck){
			if(confirm("ยืนยันการลบเซฟดาต้าสำหรับ slot"+save_slot+"")){
				clearCookie(save_slot);
				$(".ss_item:eq("+save_slot+")").text("slot "+save_slot+": no data");
			}
		}else alert("หลังจากเลือกพาร์ทเสร็จแล้ว โปรดกดปุ่ม Save");
		return false;
	}
	if(ck != "" && !confirm("ยืนยันการเซฟทับไฟล์")) return false;

	if(setCookie(save_slot, compOut())){
		$("#SS_CURRENT_VAL, .ss_item:eq("+save_slot+")").text("slot "+save_slot+": "+Assembly.robo_name.text);
		Message.set("ทำการบันทึกข้อมูลสำเร็จ");
	}else Message.set("ทำการบันทึกข้อมูลล้มเหลว", 1);
	Prop.changed = false;
});
...
$("#SS_CLEAR").click(function(){
	if(confirm("ยืนยันการลบเซฟดาต้าสำหรับ slot"+Settings.save_slot+"")){
		clearCookie(Settings.save_slot);
		$("#SS_CURRENT_VAL, .ss_item:eq("+Settings.save_slot+")").text("slot "+Settings.save_slot+": no data");
		Message.set("เซฟไฟล์ slot"+Settings.save_slot+" ถูกลบสำเร็จ");
	}
});
...
$.each(["TEC","STR"], function(i, s){
	var str='<tr><th class="th1"><div class="right">'+s+'</div><div class="left">ฟอร์ส</div></th>',
	...
	try{
		...
		function check_str(obj){
			if(!str.match(obj ? /^[a-z]\w+:(?:-?\d+|"[^\"\n]*")$/ : /^(-?\d+|"[^\"\n]*")$/))
				errmsg.push("　　การอ่านข้อมูลของ '"+str+"' ล้มเหลว");
		};

		try{
			$.each(data, function(i, c){
				if(i > MAX_DATA_SIZE) throw "ข้อมูลใหญ่เกินไป การวิเคราะห์ข้อมูลล้มเหลว";
				if(fstr){
					 str += c;
					 if(c=='"') fstr = 0;
				}else{
					...
					case '\n':
						if(errmsg.length){
							errmsg.unshift('เกิดข้อผิดพลาดสำหรับพาร์ท JSON!　'+fname+'.json Line:'+line);
							errcnt += errmsg.length-1;
							if(errcnt > MAX_ERROR_COUNT) throw "การวิเคราะห์ถูกยกเลิก เนื่องจากมีข้อผิดพลาดเยอะเกินไป";
					...
					case '}':
						...
						break;
					case '[':
						if(bc != ':') errmsg.push('　　การอ่านข้อมูลของ "'+(str || bc)+c+'" ล้มเหลว');
						brackets.push(0);
						b2++;
						str = "";
						bc = c;
	...
	this.exec = function(callback){
		...
		for(tcount=0,i=0; i<task.length; i++) (function(fname, cb){
			var webAccess = INI.webAccess && INI.accessData[fname],
				dataError = function(){
					Message.set('Error!　ไม่สามารถอ่าน '+fname+' ได้', 2);
			...
			expandData = function(data){
				if(data = evalJSON(secureJSON(data), fname)){
					Message.set("ได้รับ "+fname);
					$.each(data, function(i){
						try{
							cb.call(this, i, fname.indexOf("wp")>0 ? "wp" : fname);
						}catch(e){
							Message.set('คำเตือน!　file:'+fname+'.json　line:'+i+'　"'+this.name+'" ไม่สามารถอ่านได้เนื่องจากข้อผิดพลาด '+e, 1);
						}
					});
					Message.set("　การขยายเป็นผลสำเร็จ");
				}
				if(++tcount == task.length){
			...
			getLocal = function(){
				if(webAccess){
					Message.set("ไม่สามารถรับข้อมูลของ "+fname+" ได้, ทำการดึงข้อมูลภายในเครื่องในใช้แทน", 1);
					$.ajax({
...
function setCookie(key, val, tmp, ck) {
	...
	if(document.cookie.length + tmp.length - ck.length > 8192){
		alert('Cookie มีขนาดใหญ่เกินกว่าที่จะรับได้ \nลบข้อมูลเซฟไฟล์หรือการแสดงรายละเอียดบิ้ลด์');
		return false;
	}
...
function typeToClass(typ){
	var cls = [];
	typ.replace(/[แลนด์แอร์อาร์ทซัพ]/g, function(str){
		cls.push({"แลนด์":"riku", "แอร์":"ku", "อาร์ท":"hou", "ซัพ":"ho"}[str]);
	});
	return cls.join(" ");
};
...
function setDefault(obj, part, cmn, ss){
	...
	if(obj.type){
		var typ = "";
		if(part=="bd"){
			if(obj.type.match(/[แลนด์แอร์อาร์ทซัพ]/)) obj.type = RegExp.lastMatch;
			else obj.type = {l:"แลนด์",a:"แอร์",c:"อาร์ท",s:"ซัพ"}[obj.type] || "－";
		}else{
			var typ = obj.type.match(/[lแลนด์]/) ? "แลนด์" : "－";
			typ += obj.type.match(/[aแอร์]/) ? "แอร์" : "－";
			typ += obj.type.match(/[cอาร์ท]/) ? "อาร์ท" : "－";
			typ += obj.type.match(/[sซัพ]/) ? "ซัพ" : "－";
			obj.type = typ;
		}
...
function autoSetup(ccd){
	...
	$.each(ccd.part, function(i){
		...
		if(elm.length) sb = elm[0].ccd_sb;
			...
			Message.set("ไม่สามารถเชื่อม "+this.part.toUpperCase()+' สู่ "'+this.name+'" ได้', 1);
			return;
		}
		...
		if(this.part == "bd"){
			...
			if(pd.type.indexOf(bdpd.type) < 0) str = 'พิมพ์';
			if(SIZE_TO_NUMLIC[pd.size] !== "" &&
				Math.abs(SIZE_TO_NUMLIC[bdpd.size] - SIZE_TO_NUMLIC[pd.size]) > 1){
					str += (str ? '、' : "") + 'ขนาด';
			}
			if(str){
				Message.set('ไม่สามารถติดตั้ง "'+this.name+'"ได้เนื่องจากข้อจำกัด ของ'+str, 1);
				return;
			}
		}
	...
	if(Result.lv < j) Message.set("การอ่านคาร์ทริดจ์บางอย่างล้มเหลว", 1);
	$("#CURRENT_LV").text(Result.lv+"/10");
